...
interface Props {
  ...
  onCancel: () => void;
  onConfirmed: (result: DailymotionConfirmResult, mountName: string) => void;
}
...
export function DailyMotionReview({ token, channelName, preview, mounts, sonarrAvailable, onCancel, onConfirmed }: Props) {
  const [destMode, setDestMode] = useState<DestMode>("raw");
  const [rows, setRows] = useState<EntryRow[]>(() =>
    preview.entries.map((e) => ({
      selected: !e.already_downloaded && !e.unavailable,
      videoId: e.video_id,
  ...
  function filenamePreview(r: EntryRow): string {
    ...
    const datePart = formatUploadDate(r.uploadDate) || "unknown-date";
    const base = r.title.trim() ? `${datePart} - ${r.title.trim()}` : `${datePart} - ${r.videoId}`;
    return `${base}.mp4`;
  }
  ...
  async function handleConfirm() {
    ...
    setSubmitting(true);
    try {
      const result = await confirmDailymotion(token, {
      ...
      <div className="w-full max-w-3xl max-h-[90vh] flex flex-col rounded-lg border border-border bg-card shadow-xl">
        ...
        <div className="flex items-start justify-between px-5 pt-4 pb-3 border-b border-border">
          <div className="min-w-0">
            ...
            <p className="text-xs text-muted-foreground truncate">{preview.channel_title}</p>
            {preview.truncated && (
            ...
            <thead className="sticky top-0 bg-card">
              <tr className="text-left text-muted-foreground border-b border-border">
                <th className="py-2 pr-2 font-medium w-8">
                  <input
                    type="checkbox"
                    checked={rows.every((r) => r.selected || r.alreadyDownloaded || r.unavailable)}
                    onChange={(e) =>
                      setRows((prev) => prev.map((r) => (r.alreadyDownloaded || r.unavailable ? r : { ...r, selected: e.target.checked })))
                    }
                    disabled={submitting}
                  />
                </th>
                ...
                return (
                  <tr
                    ...
                    className={cn(
                      ...
                      (r.alreadyDownloaded || r.unavailable || !r.selected) && "opacity-50"
                    )}
                  >
                    <td className="py-2 pr-2">
                      <input
                        ...
                        disabled={r.alreadyDownloaded || r.unavailable || submitting}
                      />
                    </td>
                    {destMode === "episodes" && (
                      <>
                        <td className="py-2 pr-2">
                          <Input
                            ...
                            disabled={r.alreadyDownloaded || r.unavailable || !r.selected || submitting}
                            className={cn(
                              "h-8 w-16 px-2",
                    ...
                    {destMode === "raw" && (
                      <td className="py-2 pr-2 text-xs text-muted-foreground whitespace-nowrap">{formatUploadDate(r.uploadDate)}</td>
                    )}
                    <td className="py-2 pr-2 text-xs text-muted-foreground whitespace-nowrap">{formatDuration(r.duration)}</td>
                    <td className="py-2 pr-2">
                      {r.unavailable ? (
                        <span className="text-muted-foreground italic">unavailable (metadata fetch failed)</span>
                      ) : r.alreadyDownloaded ? (
                        <span className="text-muted-foreground italic">{r.origTitle || "(no title)"}</span>
                      ) : (
                        <>
                          <Input
                            value={r.title}
                    ...
                    <td className="py-2 text-xs text-muted-foreground max-w-[180px]">
                      <span className="block truncate" title={r.origTitle}>{r.origTitle}</span>
                    </td>
                  </tr>
                );
              })}
